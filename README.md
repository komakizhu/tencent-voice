...
设置页的“故障诊断记录”复选框用于录制一次完整复现过程：勾选后开始记录设置快照、快捷键、权限检查、连接测试、会话状态、识别结果长度、写入计数和错误码；再次取消勾选后会自动把机器可读的 JSON 导出到桌面。记录期间会同步保存脱敏的临时 journal；如果应用异常退出，下次启动会自动恢复为桌面 JSON。JSON 不包含 AppID、SecretId、SecretKey、录音、识别文字、输入框原文或剪贴板正文，但会包含当前用户、App/目标应用路径、Bundle ID 和进程号等定位信息，请在分享前确认。它只在复现期间使用，适合提交给开发者定位问题。

音频只在使用期间发送到腾讯云。应用默认不保存音频，也不保存会话日志；打开“保存会话日志”后，会持续写入当前用户的 `~/Library/Application Support/TencentVoiceMVP/sessions/` 下的 JSONL 状态记录，内容是状态、长度、计数和错误类型，不写入 SecretKey、签名 URL、音频或识别正文。它与“故障诊断记录”不同：前者是长期保留的轻量会话状态，后者是一次复现期间的临时、较完整诊断记录。

设置页的“保存设置”按钮会同时保存应用设置和腾讯凭证，但保存位置不同：应用设置保存在当前 macOS 账户的应用偏好中，凭证保存在 `/Users/Shared/TencentVoiceMVP/tencent-credentials.yaml`，会话日志不在共享目录。设置页的“数据位置”区域可以直接打开凭证共享目录、会话日志目录和诊断报告目录。
