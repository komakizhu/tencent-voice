...
struct DiagnosticReport: Codable, Equatable, Sendable {
    ...
    private static func makeFindings(
        ...
        for sessionEvents in sessionGroups {
            ...
            let diagnosticMessages: [String: String] = [
                ...
                "keyboard_state_read_retryable": "读取目标正文或坐标暂时不可用，已按阶段重读；期间未重复发送文字。",
                "keyboard_confirmed_state_unavailable": "发送前无法取得已确认的目标状态；未把读取异常当作目标改变。",
                "keyboard_write_unconfirmed": "发送后的正文或选区未在确认期限内匹配；未报告成功、回滚或补发文字。",
    ...
    private static func diagnosticDetails(_ event: SessionLogEntry) -> String {
        let keys = [
            ...
            "phase", "expectedDocumentLength", "actualDocumentLength",
